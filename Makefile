...
build: setup
	...
	docker run -it --rm -v ./:/repo ${CONTAINER_NAME} bash -c "cd /repo && ./build.sh -d quartzpro64 -r RELEASE --secure-boot"
...
build-debug: setup
	...
	docker run -it --rm -v ./:/repo ${CONTAINER_NAME} bash -c "cd /repo && ./build.sh -d quartzpro64 -r DEBUG --secure-boot"
