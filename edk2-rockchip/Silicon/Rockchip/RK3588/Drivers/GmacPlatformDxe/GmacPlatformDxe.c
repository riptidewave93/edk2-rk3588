/** @file
 *
 *  RK3588 GMAC Platform Driver
 *
 *  Initializes the GMAC controllers on the RK3588 SoC, configures PHYs,
 *  and sets up MAC addresses in the Device Tree Blob (DTB) at ReadyToBoot.
 *
...
// GMAC clock delay configuration
...
// GMAC device initialization macro
...
STATIC CONST struct {
  EFI_PHYSICAL_ADDRESS  BaseAddress;
  CONST CHAR8           *DtbPath;
  CONST CHAR8           *AcpiDevice;
} mGmacDtbMap[] = {
  { 0xfe1b0000, "/ethernet@fe1b0000", "\\_SB.MAC0" }, // GMAC0
  { 0xfe1c0000, "/ethernet@fe1c0000", "\\_SB.MAC1" }, // GMAC1
};
...
{
  UINT32 ConDlyReg = (Id == 1) ? RK3588_SYS_GRF_SOC_CON9 : RK3588_SYS_GRF_SOC_CON8;

  // Set PHY interface to RGMII
  ...
  // Configure TX clock delay
  MmioWrite32(RK3588_SYS_GRF_SOC_CON7,
              TxDelay ? RK3588_GMAC_TXCLK_DLY_ENABLE(Id) : RK3588_GMAC_TXCLK_DLY_DISABLE(Id));
  if (TxDelay) {
    MmioWrite32(ConDlyReg, RK3588_GMAC_CLK_TX_DL_CFG(TxDelay));
  }

  // Configure RX clock delay
  MmioWrite32(RK3588_SYS_GRF_SOC_CON7,
              RxDelay ? RK3588_GMAC_RXCLK_DLY_ENABLE(Id) : RK3588_GMAC_RXCLK_DLY_DISABLE(Id));
  if (RxDelay) {
    MmioWrite32(ConDlyReg, RK3588_GMAC_CLK_RX_DL_CFG(RxDelay));
  }
}

// Select GMAC clock source
STATIC VOID
...
{
  MmioWrite32(RK3588_PHP_GRF_CLK_CON1,
              Enable ? RK3588_GMAC_CLK_SELECT_IO(Id) : RK3588_GMAC_CLK_SELECT_CRU(Id));
}

// Set GMAC TX clock speed
STATIC EFI_STATUS
GmacSetTxClockSpeed (
  IN UINT32   Id,
...
// Get GMAC platform configuration
STATIC VOID
  ...
  )
{
  ...
  Hash[0] |= 0x02;
  Hash[3] &= 0xFE;
  Hash[3] |= 0x02; // Compatibility with older drivers

  ZeroMem(MacAddress, sizeof(EFI_MAC_ADDRESS));
  CopyMem(MacAddress, Hash, NET_ETHER_ADDR_LEN);
}

// Update DTB with MAC address
STATIC EFI_STATUS
UpdateDtbWithMacAddress (
  ...
  )
{
  ...
  // Find DTB path for GMAC
  for (Index = 0; Index < ARRAY_SIZE(mGmacDtbMap); Index++) {
    if (mGmacDtbMap[Index].BaseAddress == BaseAddress) {
      DtbPath = mGmacDtbMap[Index].DtbPath;
      break;
    }
  }
  ...
  // Find ethernet node
  NodeOffset = FdtPathOffset(Fdt, DtbPath);
  if (NodeOffset < 0) {
    return EFI_NOT_FOUND;
  }

  // Check if node is enabled
  CONST VOID *StatusProp = FdtGetProp(Fdt, NodeOffset, "status", NULL);
  if (StatusProp != NULL && AsciiStrCmp((CONST CHAR8 *)StatusProp, "okay") != 0) {
    return EFI_SUCCESS;
  }

  // Update MAC addresses in DTB
  Status = FdtSetProp(Fdt, NodeOffset, "local-mac-address", MacAddress->Addr, NET_ETHER_ADDR_LEN);
  if (Status != 0) {
    DEBUG((DEBUG_ERROR, "Failed to set local-mac-address for %a: %d\n", DtbPath, Status));
    return EFI_DEVICE_ERROR;
  }

  Status = FdtSetProp(Fdt, NodeOffset, "mac-address", MacAddress->Addr, NET_ETHER_ADDR_LEN);
  if (Status != 0) {
    return EFI_DEVICE_ERROR;
  }
...
// Event handler for ReadyToBoot to update DTB MAC addresses
STATIC VOID
EFIAPI
GmacUpdateDtbMacAddress (
  IN EFI_EVENT  Event,
  IN VOID       *Context
  )
{
  ...
  for (Index = 0; Index < ARRAY_SIZE(mGmacDevices); Index++) {
    ...
    MacAddress.Addr[5] += Gmac->Id;
    DEBUG((DEBUG_INFO, "Updating DTB for GMAC%u\n", Gmac->Id));
    EFI_STATUS Status = UpdateDtbWithMacAddress(&MacAddress, Gmac->BaseAddress);
    if (EFI_ERROR(Status)) {
      DEBUG((DEBUG_ERROR, "Failed to update DTB for GMAC%u: %r\n", Gmac->Id, Status));
    }
  }
}

// Initialize PHY for a GMAC device
STATIC BOOLEAN
GmacInitializePhy (
  IN GMAC_DEVICE  *Gmac
  )
{
  ...
  // Read PHY ID
  PhyRead(Gmac->BaseAddress, 0, MII_PHYIDR1, &PhyIdReg);
  PhyId = PhyIdReg << 16;
  PhyRead(Gmac->BaseAddress, 0, MII_PHYIDR2, &PhyIdReg);
  PhyId |= PhyIdReg;

  // Try each PHY initialization function
  for (PhyIndex = 0; PhyIndex < ARRAY_SIZE(mPhyInitList); PhyIndex++) {
    Status = mPhyInitList[PhyIndex](Gmac->BaseAddress, PhyId);
    if (Status == EFI_SUCCESS) {
      DEBUG((DEBUG_INFO, "GMAC%u: PHY initialized successfully\n", Gmac->Id));
      return TRUE;
    } else if (Status != EFI_UNSUPPORTED) {
      DEBUG((DEBUG_ERROR, "GMAC%u: mPhyInitList[%d] failed: %r\n", Gmac->Id, PhyIndex, Status));
    }
  }

  DEBUG((DEBUG_WARN, "GMAC%u: No valid PHY detected, skipping\n", Gmac->Id));
  return FALSE;
}

// GMAC Platform DXE entry point
EFI_STATUS
EFIAPI
  ...
  )
{
  ...
  // Create ReadyToBoot event for DTB MAC update
  ...
  if (EFI_ERROR(Status)) {
    DEBUG((DEBUG_ERROR, "Failed to create ReadyToBoot event: %r\n", Status));
    return Status;
  }

  // Generate base MAC address from OTP
  GmacGetOtpMacAddress(&MacAddress);

  // Initialize each GMAC device
  for (Index = 0; Index < ARRAY_SIZE(mGmacDevices); Index++) {
    Gmac = &mGmacDevices[Index];
    if (!Gmac->Supported) {
      continue;
    ...
    // Initialize PHY
    if (!GmacInitializePhy(Gmac)) {
      continue;
    }

    // Setup EQOS platform protocol
    ...
    DEBUG((
      DEBUG_INFO,
      "GMAC%u MAC address: %02x:%02x:%02x:%02x:%02x:%02x\n",
      Gmac->Id,
    ...
    if (EFI_ERROR(Status)) {
      DEBUG((DEBUG_ERROR, "Failed to install GMAC%u EQOS device: %r\n", Gmac->Id, Status));
    }
  }
